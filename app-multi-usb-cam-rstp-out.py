...
def main(args):
    ...
    encoder.set_property("bitrate", 1024)
    ...
    # Source element for reading from usb cams
    print("Creating Sources \n ")

    for i, device in enumerate(args[1:]):
        source = Gst.ElementFactory.make("v4l2src", f"usb-cam-source-{device}")
        if not source:
            sys.stderr.write(" Unable to create Source \n")

        # source.set_property("do-timestamp", True)

        caps_v4l2src = Gst.ElementFactory.make("capsfilter", f"v4l2src_caps_{device}")
        if not caps_v4l2src:
            sys.stderr.write(" Unable to create v4l2src capsfilter \n")

        
        jpegdec = Gst.ElementFactory.make("jpegdec", f"jpegdec_{device}")
        ...
        vidconvsrc = Gst.ElementFactory.make("videoconvert", f"convertor_src1_{device}")
        if not vidconvsrc:
            sys.stderr.write(" Unable to create videoconvert \n")

        # nvvideoconvert to convert incoming raw buffers to NVMM Mem (NvBufSurface API)
        nvvidconvsrc = Gst.ElementFactory.make("nvvideoconvert", f"convertor_src2_{device}")
        if not nvvidconvsrc:
            sys.stderr.write(" Unable to create Nvvideoconvert \n")

        caps_vidconvsrc = Gst.ElementFactory.make("capsfilter", f"nvmm_caps_{device}")
        if not caps_vidconvsrc:
            sys.stderr.write(" Unable to create capsfilter \n")

        print("Adding source elements to Pipeline \n")
        pipeline.add(source)
        pipeline.add(caps_v4l2src)
    ...
    factory.set_launch( "( udpsrc name=pay0 port=%d buffer-size=33554432 caps=\"application/x-rtp, media=video, clock-rate=90000, encoding-name=(string)%s, payload=96 \" )" % (updsink_port_num, codec))
